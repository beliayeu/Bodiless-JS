...
const fs = require('fs');
const { createDefaultContentPlugins, discoverDefaultContent } = require('./dist/DefaultContent');
...
if (process.env.BODILESS_DEFAULT_CONTENT_AUTO_DISCOVERY === '1') {
  const defaultContent = await discoverDefaultContent(process.env.BODILESS_DEFAULT_CONTENT_AUTO_DISCOVERY_DEPTH || 1);
  plugins.push(
    ...createDefaultContentPlugins(
      ...discoverDefaultContent(defaultContent),
    ),
  );
}
